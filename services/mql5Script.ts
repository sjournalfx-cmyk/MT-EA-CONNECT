...
void DrawStatusPanel()
  {
   ...
   if(ObjectFind(0, syncName) < 0)
     {
      ...
      ObjectSetInteger(0, syncName, OBJPROP_CORNER, CORNER_LEFT_UPPER);
   ...
   if(hasQueuedData)
     {
      string queueName = "EA_Queue_Text";
      if(ObjectFind(0, queueName) < 0)
        {
         ObjectCreate(0, queueName, OBJ_LABEL, 0, 0, 0);
         ObjectSetInteger(0, queueName, OBJPROP_CORNER, CORNER_LEFT_UPPER);
         ObjectSetInteger(0, queueName, OBJPROP_XDISTANCE, 20);
         ObjectSetInteger(0, queueName, OBJPROP_YDISTANCE, 84);
         ObjectSetInteger(0, queueName, OBJPROP_FONTSIZE, 7);
         ObjectSetString(0, queueName, OBJPROP_FONT, "Arial");
         ObjectSetInteger(0, queueName, OBJPROP_COLOR, clrYellow);
        }
      ObjectSetString(0, queueName, OBJPROP_TEXT, "⚠ Queued Data Pending");
     }
   else
     {
      ObjectDelete(0, "EA_Queue_Text");
     }
...
void SyncHistory()
  {
   ...
   string jsonPayload = StringFormat("{\\\"account\\\":%s,\\\"trades\\\":%s,\\\"openPositions\\\":%s}", accountJson, tradesJson, positionsJson);
   ...
   if(res == 200)
   ...
   else
     {
      ...
      if(res == 5203)
        {
         Print("Error 5203: URL not allowed in Options > Expert Advisors.");
        }
