...
app.post('/api/webhook', (req, res) => {
    ...
    tradeStore[syncKey] = { trades, account, openPositions };
...
const ConnectForm: React.FC<ConnectFormProps> = ({ onConnect }) => {
  // -- EA Bridge State --
  // UPDATED: Default to 127.0.0.1 as MT5 prefers IP over localhost
  const [syncKey] = useState('default_user');
  const [webhookUrl, setWebhookUrl] = useState('http://127.0.0.1:3001/api/webhook');
  ...
  // Auto-start listening on mount (Zero-Config)
  useEffect(() => {
    setIsWaitingForBridge(true);
  }, []);

  // Polling Logic for EA Bridge (Initial Connection Only)
  useEffect(() => {
    let interval: any;

    if (isWaitingForBridge) {
      let baseUrl = webhookUrl.replace('/webhook', '');
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (res.ok) {
            ...
            if (data.success && (data.trades || data.account)) {
              // Data Found! Stop polling and connect.
              clearInterval(interval);
              setIsWaitingForBridge(false);
              onConnect(syncKey);
            }
          } else {
            // Server reachable but maybe 404 (no data yet), which is fine
            setServerStatus('online');
          }
        } catch (e) {
  ...
  const handleDownloadEA = () => {
    ...
    element.download = "JFX JOURNAL BRIDGE.mq5";
    ...
    <div className="w-full max-w-5xl mx-auto p-6 md:p-12 animate-fade-in">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">Sync your history via local EA bridge.</p>
        </div>
      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-800/20 border border-gray-700 rounded-lg p-4">
                ...
                <input
                  ...
                  placeholder="http://127.0.0.1:3001/api/webhook"
                />
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 mb-2">Download the pre-configured EA.</p>
                  <button onClick={handleDownloadEA} className="flex items-center gap-2 text-accent hover:text-white transition-colors text-xs font-bold border border-accent/30 hover:bg-accent/10 px-3 py-1.5 rounded">
                    ...
                    Download JFX JOURNAL BRIDGE.mq5
                  </button>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <code className="block mt-2 bg-gray-950 p-2 rounded text-xs text-green-400 font-mono overflow-x-auto whitespace-nowrap">http://127.0.0.1:3001</code>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-900 to-black rounded-xl p-6 border border-gray-800 flex flex-col items-center justify-center text-center relative overflow-hidden min-h-[400px]">
            ...
            {isWaitingForBridge ? (
              <div className="flex flex-col items-center z-10 py-8 animate-fade-in">
                ...
                <div className={`mb-6 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2 ${serverStatus === 'online' ? 'bg-green-500/20 text-green-400' : serverStatus === 'offline' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                  ...
                  {serverStatus === 'online' ? 'Server Connected' : serverStatus === 'offline' ? 'Server Offline (Check Terminal)' : 'Checking Server...'}
                </div>
              ...
              <div className="w-full z-10">
                <p className="text-sm text-gray-400 mb-6">
                  Click below to start listening for trades from your local MT5.
                </p>
